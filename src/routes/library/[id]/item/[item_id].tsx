...
import { ChevronDown, ChevronUp, ArrowUp, Star, Calendar, Clock, Library as LibraryIcon, Filter } from 'lucide-solid';
import { For, Match, Show, splitProps, Switch, createSignal, onMount, onCleanup } from 'solid-js';
...
export default function Page(props: RouteSectionProps) {
  ...
  const [activeFilter, setActiveFilter] = createSignal<'all' | 'unplayed' | 'played' | 'resumable'>('all');
  ...
  const itemDetails = createJellyFinQuery(() => ({
    ...
    queryFn: async (jf) =>
      library.query.getItem(jf, params.item_id, store?.user?.Id, ['Overview','Studios','People']),
  }));
  ...
  const childrens = createJellyFinQuery(() => ({
    ...
    queryFn: async (jf) => {
      ...
      if (searchTerm() && itemDetails.data?.Type === 'Season') {
        parentId = itemDetails.data?.SeriesId || "";
      } 
      ...
      let seasons = await library.query.getItems(jf, {
        ...
        enableImage: true,
       ...(searchTerm() && { searchTerm: searchTerm(),recursive: true,  includeItemTypes: itemDetails.data?.Type === 'Series' ? ['Season'] : ['Episode'] }),
    ...
    enabled: !!itemDetails.data?.ChildCount,
  }));
  ...
  onMount(() => {
    ...
    if (contentAreaRef) {
      ...
      onCleanup(() => contentAreaRef?.removeEventListener('scroll', handleScroll));
    }
  });
  ...
  return (
    <section class="relative min-h-screen flex flex-col">
      ...
      <div class="fixed top-0 left-0 w-full h-screen">
        <img
          src={itemDetails.data?.Backdrop[0]}
      ...
      <Nav
        ...
        breadcrumbs={[
          {
            ...
            onClick: () => {
              ...
              if (itemType === 'Season' || itemType === 'Episode') {
                let seriesID = itemDetails.data?.SeriesId;
        ...
        currentPage={itemDetails.data?.Name || 'Loading...'}
        showSearch={true}
        searchValue={searchTerm()}
        onSearchChange={setSearchTerm}
      />

      <div ref={contentAreaRef} class="relative z-20 text-white px-8 py-8 flex-1 overflow-y-auto">
        <div class="flex flex-col max-w-7xl mx-auto">
          <QueryBoundary query={itemDetails}>
            {(item) => (
              <>
                {/* Hero Section */}
                <div class="space-y-4">
                  <Show
                    when={['Series', 'Movie'].includes(
                      itemDetails.data?.Type || ''
                    )}
                  >
                    <div class="max-w-xs">
                      <img
                        src={itemDetails.data?.Images['Logo']}
                        alt={itemDetails.data?.Name ?? ''}
                  ...
                  <Show when={!itemDetails.data?.Images['Logo']}>
                    <h1 class="text-2xl font-bold tracking-tight">{itemDetails.data?.Name}</h1>
                  </Show>
                  
                  {/* Metadata Section - Compact style */}
                  <div class="flex flex-wrap items-center gap-2 text-xs">
                    <Show when={itemDetails.data?.CommunityRating}>
                      <div class="flex items-center gap-1 bg-yellow-500/20 px-2 py-0.5 rounded-md">
                        ...
                        <span class="font-semibold text-yellow-400">
                          {itemDetails.data?.CommunityRating?.toFixed(1)}
                        </span>
                      </div>
                    </Show>
                    
                    <Show when={itemDetails.data?.PremiereDate}>
                      <div class="flex items-center gap-1 opacity-70">
                        <Calendar class="w-3.5 h-3.5" />
                        <span class="font-medium">
                          {new Date(itemDetails.data?.PremiereDate!).getFullYear()}
                        </span>
                      </div>
                    </Show>

                    <Show when={itemDetails.data?.RunTimeTicks}>
                      <div class="flex items-center gap-1 opacity-70">
                        <Clock class="w-3.5 h-3.5" />
                        <span class="font-medium">
                          {Math.round((itemDetails.data?.RunTimeTicks || 0) / 600000000)} min
                        </span>
                      </div>
                    </Show>

                    <Show when={itemDetails.data?.OfficialRating}>
                      <div class="px-2 py-0.5 rounded-md border border-white/30">
                        <span class="font-semibold">
                          {itemDetails.data?.OfficialRating}
                        </span>
                      </div>
                    </Show>
                  </div>
                  ...
                  <Show when={itemDetails.data?.Genres?.length}>
                    <div class="flex flex-wrap gap-1.5">
                      {itemDetails.data?.Genres?.slice(0, 4).map((genre) => (
                  ...
                  <Show when={itemDetails.data?.Overview}>
                    <div class="max-w-3xl space-y-2 pt-2">
                      <div class="flex items-center justify-between">
                        <h3 class="text-sm font-semibold opacity-70">Overview</h3>
                        <button
                      ...
                      <p 
                        class={`text-xs leading-relaxed opacity-70 transition-all duration-300 ${
                          isOverviewExpanded() ? '' : 'line-clamp-3'
                        }`}
                      >
                        {itemDetails.data?.Overview}
                      </p>
                    </div>
                  </Show>

                  {/* Additional Info - Compact Cards */}
                  <Show when={itemDetails.data?.Studios?.length || itemDetails.data?.People?.length}>
                    <div class="max-w-3xl grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                      <Show when={itemDetails.data?.Studios?.length}>
                        <div class="p-3 rounded-lg bg-white/5 border border-white/10">
                          <h4 class="text-xs font-semibold opacity-50 mb-2 uppercase tracking-wider">Studio</h4>
                          <p class="text-sm font-medium">
                            {itemDetails.data?.Studios?.map(s => s.Name).join(', ')}
                          </p>
                        </div>
                      </Show>
                      
                      <Show when={itemDetails.data?.People?.slice(0, 4).length}>
                        <div class="p-3 rounded-lg bg-white/5 border border-white/10">
                          <h4 class="text-xs font-semibold opacity-50 mb-2 uppercase tracking-wider">Cast</h4>
                          <div class="space-y-1.5">
                            {itemDetails.data?.People?.slice(0, 4).map((person) => (
                  ...
                  <QueryBoundary
                    ...
                    errorFallback={(err) => (
                      <div class="p-4 rounded-lg bg-red-500/10 border border-red-500/20">
                        <div class="text-red-400 text-sm">Error: {err?.message}</div>
                      </div>
                    )}
                    notStartedFallback={
                      <ItemsRender 
...
function ItemsRender({ parentItem, items, parentId, activeFilter, onFilterChange }: ItemsRenderProsp) {
  const FilterButton = (props: { filter: typeof activeFilter; label: string }) => (
    <button
      ...
      <Match when={parentItem?.Type === 'Series'}>
        <div class="space-y-4">
          ...
          <div class="grid 2xl:grid-cols-6 xl:grid-cols-5 lg:grid-cols-4 grid-cols-3 gap-6">
            <For each={items}>{(item) => <SeriesCard item={item} parentId={parentId} />}</For>
          </div>
        </div>
      </Match>
